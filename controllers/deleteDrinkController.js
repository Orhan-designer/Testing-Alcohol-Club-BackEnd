...
const mongoUrl =
  "mongodb+srv://Orhan:Mamedov03Danskih09@cluster0.md34d.mongodb.net/?retryWrites=true&w=majority";
...
exports.deleteDrink = (req, res) => {
  try {
    ...
    mongoClient.connect((error, client) => {
      ...
      db.collection("drinks").deleteOne(
        { _id: ObjectId(mongoId) },
        (err, result) => {
          if (err) {
            return res
              .status(400)
              .json({ error: "An error occurred while deleting the drink" });
          } else {
            const userId = req.query.userId;
            const deleteFromTable =
              "DELETE FROM drinksRating WHERE userId = '" +
              userId +
              "' AND mongoId = '" +
              mongoId +
              "'";
            ...
            mysqlDb.query(deleteFromTable, (error, deleteTableResult) => {
              if (error) {
                ...
                return res
                  .status(200)
                  .json({ message: "Drink deleted", result: result });
              }
            });
          }
        }
